...
BAUD_RATE = 9600
AVG_WINDOW = 15
RX_IP     = "—"   # overridden by serial MY_IP: / IP: messages
TX_IP     = "—"
...
event_log          = deque(maxlen=500)   # event dicts for the Event Log tab
...
stats = {
    ...
    "ber_zone":          "16QAM",   # tracks current zone for crossing detection
}

# Previous values used by callback to detect user-initiated changes
_prev_snr        = 15
...
# Lookup tables used by both listener threads and the callback
            ...
            if waiting:
                ...
                while b"\n" in buf:
                    ...
                    with lock:
                        ...
                        if line.startswith("TX_DATA:"):
                            ...
                            if msg:
                                ...
                                log_event('INFO', 'PACKET',
                                          f'TX → "{msg}" | {len(msg)*8} bits | Mod={stats["tx_mod"]} | FEC={stats["tx_fec"]}')
                            stats["tx_typing"] = ""
                        ...
                        elif line.startswith("MY_IP:") or line.startswith("IP:"):
                    ...
                    with lock:
                        ...
                        if line.startswith("BER:"):
                            try:
                                ...
                                # BER zone crossing detection
                                new_zone = ('BPSK' if avg_val >= 0.018
                                            else 'QPSK' if avg_val >= 0.006
                                            else '16QAM')
                                if new_zone != stats["ber_zone"]:
                                    sev = 'CRIT' if new_zone == 'BPSK' else 'WARN' if new_zone == 'QPSK' else 'INFO'
                                    log_event(sev, 'ADAPTIVE',
                                              f'BER zone: {stats["ber_zone"]} → {new_zone} | BER={avg_val:.4f} | SNR={stats["current_snr"]}dB')
                                    stats["ber_zone"] = new_zone
                                # High BER critical alert (throttled — one per 10 s)
                                if avg_val >= 0.018 and (not event_log or
                                        event_log[-1].get('category') != 'QUALITY' or
                                        event_log[-1].get('msg', '').startswith('High BER') is False):
                                    log_event('CRIT', 'QUALITY',
                                              f'High BER alert | BER={avg_val:.4f} | SNR={stats["current_snr"]}dB')
                            ...
                            log_event('WARN', 'ADAPTIVE',
                                      f'Modulation switch → {new_mod} | BER={stats["ber"]} | SNR={stats["current_snr"]}dB')
                            stats["mod"] = new_mod
                            if ser_tx:
                                try:
                            ...
                            if len(parts) >= 4:
                                ...
                                try:
                                    ...
                                    log_event(sev, 'PACKET',
                                              ...
                                              f'{int(eb)} err | {int(cor)} fixed | {int(res)} residual | '
                                              f'FEC={stats["fec_type"]}')
                                    if fec_fail and res / tb > 0.05:
                                        log_event('CRIT', 'QUALITY',
                                                  f'FEC decode failure | {int(res)}/{int(tb)} bits unrecovered | scheme={stats["fec_type"]}')
                            ...
                            stats["status"] = "ONLINE"
                            log_event('INFO', 'CONNECTION', f'RX online | {RX_PORT}')
                        elif line.startswith("MY_IP:") or line.startswith("IP:"):
                            ip_str = line.split(":", 1)[1].strip()
                            if ip_str:
                                stats["rx_ip"] = ip_str
        except serial.SerialException as e:
            print(f"[RX ERR] {e}"); time.sleep(1)
        except Exception as e:
            print(f"[RX ERR] {e}")
        time.sleep(0.005)
...
# Apple-style modulation colours: red=worst, orange=mid, green=best
...
def _diff_spans(text, reference):
    """Return a list of Span elements; bits that differ from reference are red."""
...
# Throughput curve toggle options
TP_CURVES = [
        ...
        html.Div(
            ...
            children=[
                html.Div([
                    ...
                    html.Div("Real-time link quality monitoring & Adaptive modulation",
                             style={'fontSize': '12px', 'color': '#aeaeb2', 'marginTop': '2px'}),
                ]),
                # Connection pills — top right
                                        ...
                                        card([
                                            ...
                                            html.Div(
                                                style={'display': 'flex',
                                                       ...
                                                       'marginBottom': '16px'},
                                                children=[
                                                    flow_box("Transmitted", 'disp-tx-data',
                                                             '#34c759'),
                                                    flow_arrow("RF"),
                                                    flow_box("Received — raw", 'disp-rx-data',
                                                             '#ff9500'),
                                                    flow_arrow("FEC"),
                                                    flow_box("Decoded — after FEC",
                                                             'disp-rx-corrected', '#0071e3'),
                                                ]
                                            ),
                        ...
                        children=[html.Div(style={'marginTop': '14px'}, children=[
                                ...
                                card([
                                    ...
                                    html.Div("Theoretical curves + live measurements (colour = mod zone)",
                                             style={'fontSize': '11px', 'color': '#c7c7cc',
                                                    'marginBottom': '8px'}),
                                    dcc.Graph(id='g-snr-ber-graph',
                                              config={'displayModeBar': False})
                                ], extra={'flex': '3'}),

                                card([
                                    section_label("Error Rate %"),
                                    dcc.Graph(id='g-error-graph',
                                              config={'displayModeBar': False})
                                ], extra={'flex': '2'}),
                            ]),

                            # Throughput vs SNR + Adaptive Modulation Selection
                            html.Div(style={'display': 'flex', 'gap': '12px',
                                ...
                                card([
                                    ...
                                    dcc.Graph(id='g-throughput',
                                              config={'displayModeBar': False})
                                ], extra={'flex': '1'}),
                                card([
                                    section_label("Adaptive Modulation Selection per SNR"),
                                    dcc.Graph(id='g-adaptive-mod',
                                              config={'displayModeBar': False})
                                ], extra={'flex': '1'}),
                            ]),

                            # FEC Comparison + Channel Impact
                            html.Div(style={'display': 'flex', 'gap': '12px',
                                            'marginTop': '12px'}, children=[
                                card([
                                    section_label("FEC Scheme Comparison — Post-FEC BER"),
                                    dcc.Graph(id='g-fec-compare',
                                              config={'displayModeBar': False})
                                ], extra={'flex': '1'}),
                                card([
                                    section_label("AWGN vs Rayleigh — Channel Impact on BER"),
                                    dcc.Graph(id='g-channel-impact',
                                              config={'displayModeBar': False})
                                ], extra={'flex': '1'}),
                            ]),

                            # Constellation Diagrams — full width
                            card([
                                section_label(
...
def snr_to_sigma(snr_db, fec_gain_db=0):
    """Convert dB SNR (+ FEC coding gain) to symbol noise sigma."""
...
def update_ui(n, snr_val, ch_val, fec_val, tp_curves, ev_filter, clear_clicks):
    ...
    # ── Config change detection (log user-initiated changes) ───
    ...
    for ser in [ser_rx, ser_tx]:
        if ser:
            try:
                ser.write(f"SNR:{snr_val}\n".encode());  ser.flush()
                ser.write(f"{ch_val}\n".encode());        ser.flush()
                ser.write(f"FEC:{fec_value}\n".encode()); ser.flush()
            except Exception as e:
                print(f"[CMD ERR] {e}")
    ...
    with lock:
        ...
        if ber_float >= 0.018:
            ber_color = '#ff3b30'
            ber_zone  = 'High BER — BPSK zone'
        elif ber_float >= 0.006:
            ber_color = '#ff9500'
            ber_zone  = 'Moderate BER — QPSK zone'
        else:
            ber_color = '#34c759'
            ber_zone  = 'Low BER — 16QAM zone'
        ...
        rx_port_open = ser_rx is not None and ser_rx.isOpen()
        tx_alive     = ser_tx is not None and tx_age < 8

        # RX 3-state: LIVE (data <2 s) | IDLE (port open, no fresh data) | OFFLINE (port closed)
        if rx_port_open and rx_age < 2:
            rx_state = "LIVE"
        elif rx_port_open:
            rx_state = "IDLE"
        else:
            rx_state = "OFFLINE"
        rx_alive = rx_port_open   # pill turns green only when LIVE, but stays non-red when IDLE
        ...
        rx_ip_str  = stats["rx_ip"]
        age_str    = f"{rx_age:.1f}s" if rx_age < 999 else "—"
        rx_text    = f"{rx_dot} RX {RX_PORT}  {age_str}"
        if rx_ip_str != "—":
            rx_text += f"  ·  {rx_ip_str}"
        ...
        # ── SNR vs BER — all modulations & all FEC modes ───────
        ...
        # Theoretical curves per modulation — No FEC (dotted reference)
        for mod, col in [('BPSK', '#ff3b30'), ('QPSK', '#ff9500'), ('16QAM', '#34c759')]:
            snr_ber_fig.add_trace(go.Scatter(
                x=snr_range_g,
                y=[_theo_ber(s, mod) for s in snr_range_g],
                mode='lines', name=f'{mod} No FEC',
                line=dict(color=col, width=1.4, dash='dot'),
            ))

        # Theoretical curves — FEC coding gain applied to BPSK baseline
        ...
        # Live measurements scatter (colour = active modulation zone)
        if snr_ber_pairs:
            ...
            snr_ber_fig.add_trace(go.Scatter(
                ...
                line=dict(color='rgba(0,113,227,0.08)', width=1),
                showlegend=False,
            ))
            snr_ber_fig.add_trace(go.Scatter(
        ...
        sel = tp_curves or TP_ALL   # fallback if dropdown is empty

        # Per-modulation, No FEC
        ...
        # With FEC — rate penalty offset by coding gain (QPSK representative)
        ...
        # Optimal transition boundaries (AWGN theory): BPSK→QPSK ~8 dB, QPSK→16QAM ~15 dB
        ...
        if snr_ber_detail:
            ...
            for mod_name, col in [('BPSK', '#ff3b30'), ('QPSK', '#ff9500'),
                                   ('16QAM', '#34c759')]:
        ...
        # ── FEC Scheme Comparison — Post-FEC BER per mod & channel
        fec_fig = go.Figure()
        ...
        # Theoretical curves per FEC (BPSK baseline)
        ...
        # Live scatter — colour=FEC, symbol=modulation
        if snr_ber_detail:
            detail = list(snr_ber_detail)
            for fec_name, col in fec_colors.items():
        ...
        # ── AWGN vs Rayleigh — Channel Impact on Post-FEC BER ──
        ch_fig = go.Figure()

        # Theoretical AWGN (BPSK)
        ...
        # Theoretical Rayleigh (BPSK): BER ≈ 0.5*(1 - sqrt(γ/(1+γ)))
        ch_fig.add_trace(go.Scatter(
            x=snr_range_g,
        ...
        # Live scatter per channel model
        ch_colors = {'AWGN': '#34c759', 'Rayleigh': '#ff9500'}
        if snr_ber_detail:
            detail = list(snr_ber_detail)
            for ch_name, col in ch_colors.items():
                pts = [(d[0], d[1]) for d in detail if d[4] == ch_name]
                if pts:
                    ch_fig.add_trace(go.Scatter(
                        x=[p[0] for p in pts], y=[p[1] for p in pts],
                        mode='markers', name=f'{ch_name} live',
        ...
        # ── Constellation Diagrams (3 panels) ──────────────────
        active_mod = stats["mod"].strip() or '16QAM'
        const_fig = make_subplots(
            rows=1, cols=3,
            subplot_titles=["No FEC", "Hamming (7,4)", "Convolutional"],
        )
        panel_sigmas = [
            snr_to_sigma(snr_val, fec_gain_db=0),    # No FEC
            snr_to_sigma(snr_val, fec_gain_db=3.5),  # Hamming (7,4)
            snr_to_sigma(snr_val, fec_gain_db=5.5),  # Convolutional
        ]
        ...
        debug = (f"SNR={snr_val}dB  ·  {ch_val}  ·  {fec_val}  ·  "
                 ...
                 f"RX {rx_age:.1f}s  TX {tx_age:.1f}s")
...
if __name__ == '__main__':
    print("\n" + "=" * 60)
    print("2.4GHz ADAPTIVE LINK CALIBRATION — CLOSED LOOP")
